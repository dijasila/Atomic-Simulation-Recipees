...
def get_rotated_mats(atoms: Atoms, mirror):
    import spglib
    ...
    if mirror:
       ...
       for ii, rot in enumerate(rotations):
        if rot[2,2]!=-1:
           new_rots.append(rot)
           new_trans.append(translations[ii])
       rotations = np.array(new_rots)
       translations = np.array(new_trans)
    print('Number of symmetry operations:', mirror, len(rotations))
...
def build_layers(atoms, cell_type, rotated_mats, transforms, rmsd_tol, mirror):
    ...
    for toplayer, (U_cc, t_c) in zip(rotated_mats, transforms):
        ...
        for pos1 in base_positions:
            for pos2 in top_positions:
                ...
                bilayer = translation(move[0], move[1], 12, toplayer, atoms, mirror)
...
def cell_specific_stacks(atoms, cell_type, rotated_mats, transforms, rmsd_tol, mirror):
    bilayers = []
    toplayers = []
    symmetries = []
    final_transforms = []
    ...
    def append_helper(x, y, top, atoms, symtup):
        U_cc, t_c = symtup
        bilayer = translation(x, y, 12, top, atoms, mirror)
...
def make_bilayer_folders(atoms, rmsd_tol, mirror=False):
    ...
    rotated_mats, transforms = get_rotated_mats(atoms, mirror)
    ...
    for mat, transl, tform, proto in zip(*things):
        ...
        name = layername(atoms.get_chemical_formula(), 2, tform[0], t, mirror)
        names.append(name)
        ...
        if mirror:
           dct['Bottom_layer_Mirror']=True
        write_json(f'{name}/translation.json', dct)

        transform_data = {'rotation': tform[0],
                          'translation': tform[1]}
        if mirror:
           transform_data['Bottom_layer_Mirror']=True          
        write_json(f'{name}/transformdata.json', transform_data)

    return names


def check_mirror(atoms):
    # I want to get from spglib if the material had mirror symmetry
    # symprec=0.01 almost no monolayer had mirror so we check symprec=0.1
    sym0 = spglib.get_symmetry(atoms, symprec=0.1, angle_tolerance=-1.0)
    rotations0 = sym0['rotations']
    translations0 = sym0['translations']

    has_mirror = False
    for ii, rot in enumerate(rotations0):
        if rot[2,2]==-1 and rot[0,2]==0 and rot[1,2]==0 and rot[2,0]==0 and rot[2,1]==0:
           has_mirror = True
        elif rot[0,2]!=0 and rot[1,2]!=0 and rot[2,0]!=0 and rot[2,1]!=0:
           raise ValueError('x or y position and z are mixed')

    print("Mirror plane:", has_mirror)
    return has_mirror
...
def main(atoms: Atoms,
         rmsd_tol: float = 0.3) -> StackBilayerResult: #ASRResult:
    ...
    # I want to get from spglib if the material had mirror symmetry
    has_mirror =  check_mirror(atoms)
   
    # The bilayers created from mirrored bottom layer are only compared with themselves
    atoms1 = atoms.copy()
    names = make_bilayer_folders(atoms1, rmsd_tol, mirror=False)
    
    if not has_mirror:
       atoms1 = atoms.copy()
       names += make_bilayer_folders(atoms1, rmsd_tol, mirror=True)
