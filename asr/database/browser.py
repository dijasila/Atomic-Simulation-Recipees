...
def create_table(
    ...
    for key in keys:
        ...
        if value is not None:
            ...
            _, desc, unit = key_descriptions.get(key, ["", key, ""])
...
def dict_to_list(dct, indent=0, char=" ", exclude_keys: set = set()):
    ...
    for key, value in dct.items():
        ...
        if isinstance(value, dict):
            lst2 = dict_to_list(
                value, indent=indent + 2, char=char, exclude_keys=exclude_keys
            )
...
def get_recipe_href(asr_name, name=None):
    ...
    link_name = (
        '<a href="https://asr.readthedocs.io/en/latest/'
...
def make_panel_description(text, articles=None):
    ...
    if articles:
        articles = bold("Relevant articles:") + ul(
...
def format_parameter_description(name, params, exclude_keys):
    ...
    string = pre(code("\n".join(lst)))
    description = bold(f"Parameters ({link_name})") + br + string

    return description


def fig(filename: str, link: str = None, caption: str = None) -> "Dict[str, Any]":
...
def matrixtable(M, digits=2, unit="", rowlabels=None, columnlabels=None, title=None):
    ...
    table = dict(type="table", rows=rows)
    return table
                    ...
                    if isinstance(item, dict):
                        if item["type"] == "table":
                            ...
                            if header in known_tables:
                                known_tables[header]["rows"].extend(item["rows"])
                                column[ii] = None
                            else:
...
def _layout(row, key_descriptions, prefix, pool):
    ...
    for record in row.records:
        ...
        missing_figures = generate_plots(context, prefix, plot_descriptions, pool)
    ...
    for paneltitle, data_sources in panel_data_sources.items():
        ...
        links = (
            bold("Relevant recipes")
            + br
            + "This panel contains information calculated "
            + "with the following ASR Recipes:"
            + br
            + ul(recipe_links)
        )
        ...
        describe_entry(
            paneltitle, description=description, title="General panel information"
        )
    ...
    misc_title, misc_columns = miscellaneous_section(row, key_descriptions, exclude)
    misc_panel = {"title": misc_title, "columns": misc_columns}
    page.append(misc_panel)

    # add links section
    link_title, link_columns = link_section(row, key_descriptions, exclude)
    link_panel = {"title": link_title, "columns": link_columns}
    ...
    for title, columns in asepage:
        columns = [[block for block in column if ok(block)] for column in columns]
        if any(columns):
    ...
    def decorator(func):
        def wrapper(result, row, key_descriptions):
            ...
            for selector in selectors:
                ...
                signs.append(sign)
                sortattrs.append(["run_specification", "parameters", *attrs])
            ...
            for value, indices in get_panels_values(representative_panels):
                ...
                for record, webpanels in zip(records, webpanels_r):
                    ...
                    row = [get_attribute(record, attrs) for attrs in sortattrs] + [
                        other_value
                    ]
                    table.append(row)
                ...
                table = table[::-1]
                html = par(bold("Convergence") + br + html_table(table, header=header))
