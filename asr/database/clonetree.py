...
def main(
    ...
    with progressbar(
        ...
        for srcdir in bar:
            ...
            contains = [
                True if name in dirfilenames else False for name in must_contain
            ]
            if not all(contains):
                continue
            ...
            not_contains = [
                True if name in dirfilenames else False for name in dont_contain
            ]
            if any(not_contains):
                continue
            ...
            for srcfile in dirfiles:
                if srcfile.is_file():
                    ...
                    if any(
                        [fnmatch.fnmatch(srcfile.name, pattern) for pattern in patterns]
                    ):
                        if destfile.is_file():
                            continue
    ...
    if not dry_run:
        with progressbar(mkdir, label=f"Creating {len(mkdir)} folders") as bar:
            for destdir in bar:
                if destdir.is_dir():
                    continue
    ...
    if copy:
        ...
        if not dry_run:
            ...
            print(f"Would create {len(mkdir)} folders and " f"symlink {len(log)} files")
