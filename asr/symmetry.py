...
def webpanel(result, row, key_descriptions):
    ...
    parameter_description = entry_parameter_description(row.data, 'asr.symmetry')
    description = ('Second order symmetry indicator of the occupied bands \n\n'
                   + parameter_description)
    datarow = [describe_entry('Symmetry indicator', description), result.indicators, result.Qc]

    summary = WebPanel(title='Summary',
                       columns=[[{'type': 'table',
                                  'header':['Electronic properties', 'Indicators', 'Qc'],
                                  'rows': [datarow]}]])
...
def main(spin_orbit: bool = True) -> Result:
    ...
    for ik, kpt in enumerate(kpts):
        #print(f'Calculating C3 eigenvalues for {X[ik]}')
