...
class SpgAtoms(Atoms):
    ...
    def set_symmetries(self, symmetries, translations):
        ...
        self.op_svv = [
            np.linalg.inv(self.cell).dot(op_cc.T).dot(self.cell)
            for op_cc in symmetries
        ]
    ...
    def log(self, forces=None, stress=None):
        ...
        if self.logfile is not None:
            ...
            if self.nsteps == 0:
                self.logfile.write(
                    " " * len(name)
                    + "  {:<4} {:<8} {:<10} ".format("Step", "Time", "Energy")
                    + "{:<10} {:<10}\n".format("fmax", "smax")
                )
...
def relax(
    ...
    dataset = spglib.get_symmetry_dataset(ats(atoms),
                                          symprec=1e-4, angle_tolerance=0.1)
    atoms = SpgAtoms.from_atoms(atoms)
    atoms.set_symmetries(
        symmetries=dataset["rotations"], translations=dataset["translations"]
    )
    if dftd3:
        calc = DFTD3(dft=dft)
    else:
    ...
    spgname, number = spglib.get_spacegroup(
        ats(read("unrelaxed.json", parallel=False)),
        symprec=1e-4, angle_tolerance=0.1
    ).split()
    ...
    for _ in runner:
        ...
        spgname2, number2 = spglib.get_spacegroup(
            ats(atoms), symprec=1e-4, angle_tolerance=0.1
        ).split()
...
def main(
    ...
    msg = (
        "You cannot already have a structure.json file "
    ...
    if calculatorname == "gpaw":
        ...
        if nd == 2:
            assert not atoms.get_pbc()[2], (
                "The third unit cell axis should be aperiodic "
                "for a 2D material!"
            )
