...
def get_energy(base, top, h, t_c, calc, callback, memo, mirror):
    ...
    # separation
    atoms = translation(tx, ty, h[0], top, base, mirror)
    atoms.calc = calc
...
def main(atoms: Atoms,
    ...
    ### If there is a TM3d atom in the list, Read and use the magnetic moments from the monolayers.
    #TM3d_atoms = ['V', 'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu']
    #if any(x in atoms.symbols for x in TM3d_atoms):

    if "magmom" in read_json("../structure.json")[1]: 
    ...
    start_structure = translation(t_c[0], t_c[1], d0, top_layer, atoms, mirror)
    ...
    calc = calc_setup(settings, start_structure)

    def energy_fn(h):
        return get_energy(atoms, top_layer,
                          h, t_c, calc,
                          callback_fn, energy_curve, mirror)
