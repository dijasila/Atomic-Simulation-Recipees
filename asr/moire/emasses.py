...
@command(module='asr.moire.emasses',
         requires=['results-asr.moire.magnetic_anisotropy.json'],
         dependencies=['asr.structureinfo',
                       'asr.moire.magnetic_anisotropy'],
                       #'asr.gs'],
...
@option('-s', '--settings', help='Settings for the two refinements',
        type=DictStr())
def refine(gpwfilename: str = 'gs_scs.gpw',
           settings: dict = {
               'erange1': 250e-3,
               'nkpts1': 19,
               'erange2': 1e-3,
...
def preliminary_refine(gpw='gs_scs.gpw', soc=True, bandtype=None, settings=None):
...
def nonsc_sphere(gpw='gs_scs.gpw', fallback='gs_scs.gpw', soc=False,
...
@command('asr.moire.emasses',
         requires=['em_circle_vb_soc.gpw', 'em_circle_cb_soc.gpw',
                   'results-asr.structureinfo.json',
                   #'results-asr.gs.json',
                   'results-asr.moire.magnetic_anisotropy.json'],
         dependencies=['asr.moire.emasses@refine',
                       #'asr.gs@calculate',
                       #'asr.gs',
                       'asr.structureinfo',
                       'asr.moire.magnetic_anisotropy'],
...
@option('--gpwfilename', type=str,
        help='GS Filename')
def main(gpwfilename: str = 'gs_scs.gpw') -> Result:
    from asr.utils.gpw2eigs import gpw2eigs
    from ase.dft.bandgap import bandgap
    from asr.magnetic_anisotropy import get_spin_axis
    import traceback
