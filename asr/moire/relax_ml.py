...
By defaults read from "unrelaxed_ml.json" from disk and relaxes
...
def relax(atoms, tmp_atoms_file, emin=-np.inf, smask=None,
...
@command('asr.relax_ml',
         creates=['structure_ml.json'],
         returns=Result)
@option('-a', '--atoms', help='Atoms to be relaxed.',
        type=AtomsFile(), default='unrelaxed_ml.json')
@option('--tmp-atoms', help='File containing recent progress.',
        type=AtomsFile(must_exist=False), default='relax_ml.traj')
@option('--tmp-atoms-file', help='File to store snapshots of relaxation.',
        default='relax_ml.traj', type=str)
...
def main(atoms: Atoms,
         ...
         tmp_atoms_file: str = 'relax_ml.traj',
         d3: bool = False,
         fixcell: bool = False,
    ...
    calc = mace_mp(model="large", dispersion=d3, default_dtype="float64")
    # Relax the structure
    atoms = relax(atoms, tmp_atoms_file=tmp_atoms_file,
                  fixcell=fixcell,
