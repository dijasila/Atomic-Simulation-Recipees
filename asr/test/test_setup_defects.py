...
def test_setup_defects_supercell():
    ...
    for i in range(len(defect_ref.get_scaled_positions())):
        assert defect_ref.get_scaled_positions()[i][0] == approx(
            defect_rec.get_scaled_positions()[i][0], abs=1e-2)
        assert defect_ref.get_scaled_positions()[i][1] == approx(
            defect_rec.get_scaled_positions()[i][1], abs=1e-2)
        assert defect_ref.get_scaled_positions()[i][2] == approx(
            defect_rec.get_scaled_positions()[i][2], abs=1e-2)
    for i in range(len(defect_ref.get_cell())):
        assert defect_ref.get_cell()[i][0] == approx(
            defect_rec.get_cell()[i][0], abs=1e-2)
        assert defect_ref.get_cell()[i][1] == approx(
            defect_rec.get_cell()[i][1], abs=1e-2)
        assert defect_ref.get_cell()[i][2] == approx(
            defect_rec.get_cell()[i][2], abs=1e-2)
