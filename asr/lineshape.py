...
class Lineshape:
    ...
    def __init__(self, ground, excited, phonon, sigma = 0.01, delta_t = 0.01, N = 18, gamma = 0.05):
    ...
    def broadening(self, omega_i, omega_0, sigma):
        ...
        gauss_i = 1 / (sigma * sqrt(2 * pi)) * np.exp(-1 / 2 * ((omega_i - omega_0) / sigma)**2)
    ...
    def get_partial_hr(self):
        ...
        q_l = ((modes_lav * delta_R_av).sum(axis=-1) * np.sqrt(self.masses_a)).sum(axis=-1)
    ...
    def plot_elph_function(self, ax, filename=None):
        ...
        #markerline, stemlines, baseline = ax.stem(omega_l, S_l, 'k', basefmt=' ', markerfmt=' ', use_line_collection=False)
        #plt.setp(stemlines, color='k', linewidth=1.5)

        ax.plot(self.omega_k, S_k * np.max(S_l) / np.max(S_k), '-k')
    ...
    def plot_spectral_function(self, ax, transition='emission', ZPL=2.1, color='-C2', label='Generating function', filename=None):

        Aw_k = self.get_spectral_function()

        if transition == 'emission':
            ax.plot(-self.omega_k + np.max(self.omega_k)/2 + ZPL, Aw_k, color, lw=2., label=label)
        if transition == 'absorption':
            ax.plot(self.omega_k - np.max(self.omega_k)/2 + ZPL, Aw_k, color, lw=2., label=label)
        #filename = "sajid.dat"
       #print("Writing", filename)
        #f = open(filename,'w')
        #for i in range (len(self.omega_k)):
        #    f.write('%.5e   %.5e\n' % (-self.omega_k[i] + np.max(self.omega_k)/2 + ZPL, Aw_k[i]))
        #f.close()
        dict={
        "Energy": -self.omega_k + np.max(self.omega_k)/2 + ZPL,
        "PL Intensity": Aw_k,}
