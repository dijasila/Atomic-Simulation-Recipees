...
def get_cb_vb_surface(
        calc: typing.Union[GPAW, None] = None,
        ef = None, eigenvalues = None,
        kpts = None,
        all_bz: bool = True,
        soc: bool = False):
...
class Result(ASRResult):
    ...
    key_descriptions = {'deformation_potentials_nosoc': 'Deformation potentials under different types \
                        ...
                        'deformation_potentials_soc': 'Deformation potentials under different types \
                         of deformations (xx, yy, zz, yz, xz, xy) at each k-point, with SOC',
...
@option('--all-ibz', is_flag=True, 
        help="Calculate deformation potentials on all the irreducible Brillouin zone.", type=bool)
def main(strain_percent = 1.0,
    ...
    results = {}
    # If requested, We obtain the deformation potentials at all the irreducible brillouin zone k-points.
    if all_ibz:
        calc_nostrain = GPAW('gs.gpw')
        atoms = calc_nostrain.get_atoms()
        kpts = calc_nostrain.get_ibz_k_points()

    # Otherwise we use only the special points
    else:
        calc = GPAW('gs.gpw')
        atoms = calc.get_atoms()
        specpts = atoms.cell.bandpath(pbc=atoms.pbc).special_points
        kpts = [specpts[i] for i in specpts.keys()]
        results.update({'kpt_labels': [i for i in specpts.keys()]})
